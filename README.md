...
1. выполняем
   ...
   ```
2. копируем содержимое файла
3. открываем `about:debugging` -> `This Firefox` -> `Inspect` в карточке загруженного расширения -> вкладка `Storage` -> `Extension Storage` -> вставляем в поле `Value` ключа `blockedChannels`
