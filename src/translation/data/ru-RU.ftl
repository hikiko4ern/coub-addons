...
i-am-a-developer-tooltip = Тут обитают драконы. Добавляет функционал, предназначенный только для разработчиков. Такой функционал может быть изменён или удалён в любой момент без уведомления, а его использование потенциально может что-то сломать или безвозвратно стереть настройки расширения
